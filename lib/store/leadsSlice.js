...
export const selectPaginatedLeads = (state) => {
  const filtered = selectFilteredLeads(state)
  const { page, perPage } = state.leads.pagination
...
export const selectUniqueFilterOptions = (state) => {
...
export const selectLoadingState = (state) => ({
  ...
  isLoadingMore: state.leads.isLoadingMore,
  isFullyLoaded: state.leads.isFullyLoaded,
  loadedCount: state.leads.loadedCount,
  total: state.leads.total,
  progress: state.leads.total > 0 ? Math.round((state.leads.loadedCount / state.leads.total) * 100) : 0,
})
