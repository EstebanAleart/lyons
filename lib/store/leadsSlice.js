...
const CHUNK_SIZE = 1000 // Leads por request
  ...
  reducers: {
    ...
    updateLeadEtapa: (state, action) => {
      ...
      if (leadIndex !== -1) {
        ...
        // Si la etapa es 'convertido', marcar como cliente
        if (etapa === 'convertido') {
          state.items[leadIndex].esCliente = true
        }
      }
...
export const { setFilter, clearFilters, setPage, setPerPage, resetLeads, markLeadAsClient, unmarkLeadAsClient, updateLeadEtapa } = leadsSlice.actions
