...
const CHUNK_SIZE = 500 // Clientes por request
...
export const { setFilter, clearFilters, setPage, setPerPage, resetClientes } = clientesSlice.actions
...
export const selectFilteredClientes = createSelector(
  ...
  (items, filters) => {
    return items.filter(cliente => {
      ...
      if (filters.localidad !== 'Todos' && cliente.localidad !== filters.localidad) {
        return false
      }
